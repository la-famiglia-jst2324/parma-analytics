...
def mock_get_db():
    db = MagicMock()
    db.execute.return_value = MagicMock() #redundant
    return db


@patch("parma_analytics.db.prod.engine.get_db", new_callable=mock_get_db)
@patch("parma_analytics.bl.source_measurement_bll.list_source_measurements_bll")
...
@patch("parma_analytics.bl.source_measurement_bll.list_source_measurements_bll")
@patch("parma_analytics.db.prod.engine.py", new_callable=mock_get_db)
