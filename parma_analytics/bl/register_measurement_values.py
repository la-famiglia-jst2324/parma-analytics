...
def register_values(
    source_measurement_id: int, company_id: int, value: typing.any, timestamp: datetime
) -> Optional[int]:
    ...
    Returns:
    int: Currently returns None. This should be updated when the function is implemented.
    """
    try:
        ...
        if company_measurement is None:
            company_measurement = create_company_measurement_query(
                session,
                {"sourceMeasurementId": source_measurement_id, "companyId": company_id},
            )
        ...
        if source_measurement is None:
            raise ValueError(
                "Source measurement with id {} does not exist.".format(
        ...
        if handle_type is not None:
            ...
            raise ValueError("Invalid type: {}".format(source_measurement.type))
