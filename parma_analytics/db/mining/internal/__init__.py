...
from parma_analytics.db.mining.internal.storage import save_document
...
def _traverse(
    ...
    if isinstance(current_item, Collection):
        ...
        for doc_instance_name, doc_instance in current_item.instances.items():
            new_doc = _create_doc_from_template(
                engine, current_ref, current_item, doc_instance
            )
            if new_doc is None:
                continue
            for col_name, col in current_item.collections.items():
...
def _create_collection(
    ...
    Does not commit the changes but returns the collection reference.
...
def _create_doc(
    ...
    Args:
        ...
        doc: The doc to create.
    """
    parent_collection.list_documents()
    fs_doc: firestore_types.DocumentReference = parent_collection.document(doc.name)

    return save_document(fs_doc, doc.values)
...
def _create_doc_from_template(
    ...
    return _create_doc(parent_collection, doc)
