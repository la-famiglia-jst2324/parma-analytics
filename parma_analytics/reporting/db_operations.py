...
def fetch_channel_ids(user_ids: list[int]) -> list[int]:
    ...
    with get_session() as db:
        query = text(
            "SELECT channel_id FROM notification_subscription "
...
def fetch_notification_destinations(
    channel_ids: list[int], service_type: str
) -> list[str]:
    """Fetch notification destinations for a given list of channel ids.

    Args:
        channel_ids: list of channel ids.
        service_type: type of the service. This parameter specifies
                      the type of service for which the notifications
                      are being fetched.
